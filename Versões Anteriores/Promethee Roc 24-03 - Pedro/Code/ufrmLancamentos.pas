...
begin
  FileArquivo.SaveToFile('C:\Pedro - CDSID\PROMETHEE' + (FileArquivo.Filename));
  TIWAdvWebGridExcelIO1.XLSImport('C:\Pedro - CDSID\PROMETHEE' +
      (FileArquivo.Filename));
...
begin
  ...
  with UserSession.ZQuery1 do
    begin
      ...
      begin
      SQL.Text :=
        'INSERT INTO criterio (nome_criterio, sigla_criterio, direcao_criterio, ID_problema) VALUES ("' + CritName[i] + '","' + CritSigla[i] + '","' + IntToStr(rdgrpdirecaocriterio.ItemIndex) + '","' + IntToStr(TIWUserSession(WebApplication.Data).CurrentID_problem) + '")';
      ExecSQL;
      end; // with usersession
      end; // for i
      ...
      begin
        ...
        begin
          edt := TIWEdit(rgnDisplaymatrizconsequencia.FindComponent
            ...
            TIWUserSession(WebApplication.Data).Mconseq[i,j] := strtofloat(edt.text);
            TIWUserSession(WebApplication.Data).simMconseq[i,j] := strtofloat(edt.text);
            SQL.Text :=
...
begin
  ...
  begin
    ...
    With TIWUserSession(WebApplication.Data) do
    begin
      Critname[lstbxalternativa.ItemIndex] := edtnomealternativa.Text;
...
begin
  ...
  begin
    ...
    peso[Ncrit-1] := 0;
    simpeso[Ncrit-1] := 0;
    prefdir[Ncrit-1] := rdgrpdirecaocriterio.ItemIndex;
    end;
