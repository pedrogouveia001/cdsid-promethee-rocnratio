...
uses
  ...
  IWBaseHTML40Component, ShellAnimations;
...
begin
  ...
  begin
    ...
    begin
    if (edtlimiarp.Text = '') or (edtlimiari.text = '') then
    begin
    auxp := 1;
    auxi := 0;
    end
    ...
    begin
    auxp := strtofloat(TrocaPtoPVirg(edtlimiarp.Text));
    auxi := strtofloat(TrocaPtoPVirg(edtlimiari.Text));
    end;
      ...
      begin
        ...
        begin
          SQL.Text := 'UPDATE criterio SET limiar_g_criterio="' +
            ...
            (critcode[cmbbxCriterios.ItemIndex]);
          ExecSQL;
          auxg := strtofloat(TrocaPtoPVirg(edtlimiarg.Text));
...
begin
  ...
  for j := 0 to TIWUserSession(WebApplication.Data).Ncrit - 1 do
  begin
  auxpeso := TrocaPtoPVirg(edt[j].Text);
    if auxpeso = '' then
    begin
