      ...
      for i := 1 to TIWUserSession(WebApplication.Data).Ncrit do
        ...
        begin
            TIWUserSession(WebApplication.Data).MConseq[j - 1, i - 1] :=
              StrToFloat(TrocaPtoPVirg(FieldByName('valor_performance').AsString));
...
begin
  ...
  FileArquivo.SaveToFile('C:\Pedro - CDSID\PROMETHEE\PROMETHEE ROCnRATIO\Promethee Roc 30-03 - Pedro\Planilhas' + (FileArquivo.Filename));
  TIWAdvWebGridExcelIO1.XLSImport('C:\Pedro - CDSID\PROMETHEE\PROMETHEE ROCnRATIO\Promethee Roc 30-03 - Pedro\Planilhas' +
      (FileArquivo.Filename));
  ...
  // número total de linhas preenchidas
   ...
   begin
    ...
    Open;
    First;
    idcrit := FieldByName('ID_criterio').AsInteger;
    close;
  ...
  //Setando os elementos dos criterios
   ...
   peso[i-1] := 0;
   simpeso[i-1] := 0;
   prefdir[i-1] := strtoint(grdImport.CELLS[i, 1]);
   critcode[i - 1] := idcrit;
   inc(idcrit);
   ...
   for j := 1 to TIWUserSession(WebApplication.Data).Nalt do
    ...
    begin
      for j := 1 to TIWUserSession(WebApplication.Data).Nalt do
      begin
      Mconseq[j-1,i-1] := strtofloat(TrocaPtoPVirg(grdImport.CELLS[i, j + 7]));
      simMconseq[j-1,i-1] := strtofloat(TrocaPtoPVirg(grdImport.CELLS[i, j + 7]));
        UserSession.ZQuery1.SQL.Text :=
      ...
      begin
        ...
        begin
          ...
          edt.Enabled := false;
            ...
            TIWUserSession(WebApplication.Data).Mconseq[i,j] := strtofloat(trocaptopvirg(edt.text));
            TIWUserSession(WebApplication.Data).simMconseq[i,j] := strtofloat(trocaptopvirg(edt.text));
            SQL.Text :=
...
begin
   WebApplication.SendFile
    ('C:\Pedro - CDSID\PROMETHEE\PROMETHEE ROCnRATIO\Promethee Roc 28-03 - Pedro\Promethee Roc 26-03 - Pedro\Code\Files\template_new' + '.xls', true);
  WebApplication.ShowMessage('Template downloaded successfully', smAlert);
end;
...
procedure TfrmLancamentos.IWLabel7AsyncClick(Sender: TObject;
  EventParams: TStringList);
begin
WebApplication.SendFile('C:\Pedro - CDSID\PROMETHEE\PROMETHEE ROCnRATIO\Promethee Roc 28-03 - Pedro\Promethee Roc 26-03 - Pedro\Code\Files\carpurchase_example' +
      '.xls', true);
  WebApplication.ShowMessage('Example downloaded successfully', smAlert);
end;
