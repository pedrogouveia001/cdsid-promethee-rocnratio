...
begin
  ...
  begin
  //colunasumario := grdflows.ColumnCount;
  end;
  with TIWUserSession(WebApplication.Data) do
  begin
    ...
    for i := 1 to Nalt do
      begin
        ...
        Cells[i, 1 + Nalt + 16] := TIWUserSession(WebApplication.Data).AltName[TIWUserSession(WebApplication.Data).finalrankingsim[i - 1] - 1];
        Cells[i, 2 + Nalt + 16] := FloatToStr
          (RoundTo(TIWUserSession(WebApplication.Data).PositiveFlowSim[finalrankingsim[i - 1] - 1], -4));
        Cells[i, 3 + Nalt + 16] := FloatToStr
          (RoundTo(NegativeFlowSim[finalrankingsim[i - 1] - 1], -4));
        Cells[i, 4 + Nalt + 16] := FloatToStr(RoundTo(NetFlowSim[finalrankingsim[i - 1] - 1], -4));
      end;
    end;
    ...
    for i := 0 to grdConseqMatRangeTable.RowCount - 1 do
      for j := 0 to grdConseqMatRangeTable.ColumnCount - 1 do
        grdExportacao.Cells[j, i + Nalt + 21 + Nalt  +
          grdWeightRangeTable.RowCount] := grdConseqMatRangeTable.Cell[i, j].Text;
    ...
    for i := 0 to grdtautable.RowCount - 1 do
      for j := 0 to grdtautable.ColumnCount - 1 do
        grdExportacao.Cells[j, i + Nalt + 24 + Nalt +
          grdWeightRangeTable.RowCount + grdConseqMatRangeTable.RowCount] :=
   ...
   aux := 'Problem - ';
    for i := 9 to length(lblnameproblem4.Caption) - 1 do
    begin
      if lblnameproblem4.Caption[i] = '-' then
      break;

      aux := aux + lblnameproblem4.Caption[i];
    end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        grdSimulationReport.Cell[i, 1].Text := TIWUserSession(WebApplication.Data).AltName[TIWUserSession(WebApplication.Data).finalrankingsim[i - 1] - 1];
        for j := 1 to Nalt do
    ...
    for i := 0 to Nalt-1  do
    begin
    series[0].AddXY(2*i, RoundTo(rankchanges[i, 0] / NumCases * 100, -4),
          FloatToStr(RoundTo(rankchanges[i, 0] / NumCases * 100, -4)), clwebblue);
    series[1].AddXY(2*i, RoundTo(rankchanges[i, 1] / NumCases * 100, -4),
          FloatToStr(RoundTo(rankchanges[i, 1] / NumCases * 100, -4)), clwebred);

        chart.BottomAxis.Items.Add(2*i, AltName[finalrankingsim[i]-1]);
      ...
      tbsht.VertScrollBar.Visible := false;
      tbsht.HorzScrollBar.Visible := false;
      tbsht.Title := '  ' + AltName[finalrankingsim[i] - 1] + '  ';
      ...
      begin
        series[0].AddXY
          (j, RoundTo(TIWUserSession(WebApplication.Data).rankchangesdetailed[i, j] / NumCases * 100, -4),
          FloatToStr(RoundTo(TIWUserSession(WebApplication.Data).rankchangesdetailed[i, j] / NumCases * 100, -4)
            ), clwebblue);
        chart.BottomAxis.Items.Add(j, IntToStr(j+1));
        if i = j then
...
begin
  ...
  begin
    ...
    if TIWUserSession(WebApplication.Data).currentinstance = 1 then
      auxiliar[i, 1] := IntToStr(TIWUserSession(WebApplication.Data).finalranking[i - 1])
    else
      auxiliar[i, 1] := IntToStr(TIWUserSession(WebApplication.Data).finalrankingsim[i - 1]);
    // seção
  end;
  ...
  z := StrToFloat(TIWUserSession(WebApplication.Data)
   ...
   TIWUserSession(WebApplication.Data).zmean := TIWUserSession(WebApplication.Data).zmean + abs(z);
  // z1:=roundto(z,-6);
