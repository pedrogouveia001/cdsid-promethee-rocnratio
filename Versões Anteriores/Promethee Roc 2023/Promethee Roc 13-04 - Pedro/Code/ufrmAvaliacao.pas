...
type
  TGrid = class(TIWGrid);
    TfrmAvaliacao = class(TIWAppForm)RgCorpo: TIWRegion;
    ...
    lblnameuser3: TIWLabel;
    lblnameproblem3: TIWLabel;
    IWTimer1: TIWTimer;
...
begin
  ...
  for xcrit := 1 to TIWUserSession(WebApplication.Data).Ncrit do
  begin
    if TIWUserSession(WebApplication.Data).PrefDir[xcrit - 1] = 1 then
    begin
    ...
    else if TIWUserSession(WebApplication.Data).PrefDir[xcrit - 1] = 0 then
    begin
      case TIWUserSession(WebApplication.Data).tipocrit[xcrit - 1] of
        1: // critério usual
          begin
...
begin
  ...
  begin
    ...
    for i := 1 to Nalt do
      finalranking[i - 1] := finalrankingsim[i - 1];
...
begin
  ...
  begin
    ...
    grdExportacao.Cells[0, 0] := lblnameuser3.Caption;
    grdExportacao.Cells[0, 1] := lblnameproblem3.Caption;
    ...
    begin
      ...
      begin
        ...
        grdExportacao.Cells[j, 7] := IntToStr(PrefDir[j - 1]);
    ...
    aux := 'Problem - ';
    for i := 9 to length(lblnameproblem3.Caption) - 1 do
    begin
      if lblnameproblem3.Caption[i] = '-' then
      break;

      aux := aux + lblnameproblem3.Caption[i];
    end;
    TIWAdvWebGridExcelIO1.AdvWebGrid := grdExportacao;
