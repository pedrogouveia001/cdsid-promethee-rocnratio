...
begin
  // Inherited;
 { WebApplication.SendFile('C:\Pedro - CDSID\PROMETHEE\Planilhas\carpurchase_example' +
      '.xls', true);  }
 FileArquivo.Update;
  WebApplication.SendFile(
    'C:\inetpub\SU_PXOMO_WF1\Files\carpurchase_example.xls', true,
    'carpurchase_example.xls', 'carpurchase_example.xls');
  WebApplication.ShowMessage('Example downloaded successfully', smAlert);
    FileArquivo.Create(Self);
  { LockOnSubmit := false;
    Release;
    FileArquivo.Refresh;
    Inherited;
...
begin
  ...
  FileArquivo.SaveToFile('C:\inetpub\SU_PXOMO_WF1\Files\' + (FileArquivo.Filename));
  TIWAdvWebGridExcelIO1.XLSImport('C:\inetpub\SU_PXOMO_WF1\Files\' +
      (FileArquivo.Filename));
  ...
  //Setando os elementos dos criterios
   ...
   for j := 1 to TIWUserSession(WebApplication.Data).Nalt do
    ...
    begin
      for j := 1 to TIWUserSession(WebApplication.Data).Nalt do
      begin
      Mconseq[j-1,i-1] := strtofloat(TrocaVirgPPto(grdImport.CELLS[i, j + 7]));
      simMconseq[j-1,i-1] := strtofloat(TrocaVirgPPto(grdImport.CELLS[i, j + 7]));
        UserSession.ZQuery1.SQL.Text :=
      ...
      begin
        ...
        begin
          ...
          edt.Enabled := false;
            ...
            TIWUserSession(WebApplication.Data).Mconseq[i,j] := strtofloat(TrocaVirgPPto(edt.text));
            TIWUserSession(WebApplication.Data).simMconseq[i,j] := strtofloat(TrocaVirgPPto(edt.text));
            SQL.Text :=
...
var
  ...
  Texto: string;
begin
  if ((edtnomealternativa.Text = '') or (TIWUserSession(WebApplication.Data).Nalt = 0)) then
  begin
    WebApplication.ShowMessage(
...
procedure TfrmLancamentos.IWLabel7AsyncClick(Sender: TObject;
  EventParams: TStringList);
begin
//WebApplication.SendFile('C:\inetpub\prometheeroc\Files\carpurchase_example' +
     // '.xls', true);
  //WebApplication.ShowMessage('Example downloaded successfully', smAlert);
